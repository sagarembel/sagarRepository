...
<%
	...
	try {
		...
		ResultSet rs = stmt.executeQuery("select c.ItemName, c.Quantity, c.SalePrice, c.Discount, c.TotalAmount, c.GrossTotal, c.Date, c.totalperitem, c.Gst, c.Igst, c.TaxAmount, c.HsnSacNo , s.contact_no, s.first_name, s.last_name, c.pending_bill_payment, perProductdisPer, taxAmtAfterDiscount, billTime, FkSaleEmployeeId, sb.subcat_name, c.BarcodeNo, c.payment_mode, c.payment_mode, ud.userName from creditcustomerbill c left join customer_details s ON c.fkCrediCustId=s.pk_customer_id JOIN sub_categories sb on c.fkSubCatId=sb.pk_subcat_id join user_details ud on c.cEmpIdFk = ud.pk_user_id	where c.BillNo = "+billno+" AND c.Quantity > 0");
		...
		rs.next();
			...
			headerTable_cell22 = new PdfPCell(new Phrase("PEHENAVA", font14Bold));
			...
			headerTable_cell22 = new PdfPCell(new Phrase("Marwad Peth, Baramati.", font8Bold));
			headerTable_cell22.setHorizontalAlignment(Element.ALIGN_CENTER);
			headerTable_cell22.setPaddingBottom(2);
			headerTable_cell22.setBorder(Rectangle.NO_BORDER);
			headertable22.addCell(headerTable_cell22);
		...
		headerTable_cell21 = new PdfPCell(new Phrase("Bill No: "+billno, font8));
		...
		headerTable_cell21 = new PdfPCell(new Phrase("Time:"+billTime,font8));
...
while(rs.next())
{
	...
	String totalAmt2 = String.valueOf(rs.getDouble("totalperitem"));
	table_cell = new PdfPCell(new Phrase(" " + totalAmt2, font8));
	table_cell.setHorizontalAlignment(Element.ALIGN_CENTER);
	table_cell.setBorder(Rectangle.RIGHT);
	table.addCell(table_cell);
	...
	finalGrossTotal = finalGrossTotal + Double.parseDouble(totalAmt2);
	System.out.println("TOTAL AMOUNT ===================================== "+finalGrossTotal);
...
{ 
	...
	table_cell = new PdfPCell(new Phrase(""+finalGrossTotal, font8Bold));
	table_cell.setHorizontalAlignment(Element.ALIGN_CENTER);
